...
public class GetAgents {
	...
	public static ArrayList<Agent> getAgentList() throws IOException, InterruptedException {
		...
		for (int i = 0; i < lines.length; i++) {
			if (lines[i].contains("|     ")) {
				...
				String proAcd = lines[i].substring(75, 80).trim();
				...
				if (proAcd.length() > 0) {
					...
					System.out.println("AgentIpAddress = " + agentIpAddress);

					if (!agentIpAddress.equals("0.0.0.0")) {
						if (agentIpAddress.contains("10.100.201") || agentIpAddress.contains("10.28.34")
								|| agentIpAddress.contains("10.28.101")) {
							office = true;
						}
					}
				}
				if (!agentExtension.equals("4003") && !agentExtension.equals("4008")) {
					...
					agentList.add(new Agent(agentFullName, agentExtension, agentStation, agentPG, loggedOn, agentState,
							agentWithdrawalType, agentDynamicState, office, agentIpAddress));
				}
			}
		}
		...
		for (Agent agent : agentList) {
			...
			if (agentName.equals("Steve Dams") || agentName.equals("Steven Deferme")
					|| agentName.equals("Anthony Holderbeke") || agentName.equals("Adrian Stancu")
					|| agentName.equals("Tim Willems") || agentName.equals("Nicolas Dricot")) {
	...
	public static String getAgentFullName(String agentName) {
		...
		case "SCHIETTECAT  Chr":
			...
			break;
		case "DRICOT AGENT Nic":
			agentFullName = "Nicolas Dricot";
			break;
		default:
	...
	public static void main(String[] args) {
		try {
			...
			/*
			 * for(Agent agent : agentList) { System.out.format(format,
			 * agent.getAgentExtension(), agent.getAgentName() ,
			 * String.valueOf(agent.getLoggedOn()), agent.getAgentStation(),
			 * agent.getAgentPG(), agent.getAgentState(), agent.getAgentWithdrawalType(),
			 * agent.getAgentDynamicState(), agent.getOffice(), agent.getIpAddress());
			 * 
			 * }
			 */
