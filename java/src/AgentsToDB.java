...
public class AgentsToDB {
	...
	public static void agentsToDB() {
		try {
			...
			for (Agent agent : agentList) {
				System.out.format(format, agent.getAgentExtension(), agent.getAgentName(),
						String.valueOf(agent.getLoggedOn()), agent.getAgentStation(), agent.getAgentPG(),
						agent.getAgentState(), agent.getAgentWithdrawalType(), agent.getAgentDynamicState(),
						agent.getOffice());
			}
	...
	public static void createAgent(Agent agent) {
		...
		String statement = "INSERT INTO agents (name, extension, station, logged_on, pg, state, withdrawal, dynamic_state, office) "
				+ "VALUES('" + agent.getAgentName() + "', " + agentExtension + ", " + agentStation + ", " + loggedOn
	...
	public static void updateAgent(Agent agent) {
		...
		String statement = "UPDATE agents SET station=" + agentStation + ", logged_on=" + loggedOn + ", pg='"
				+ agent.getAgentPG() + "', state='" + agent.getAgentState() + "', withdrawal='"
				+ agent.getAgentWithdrawalType() + "', dynamic_state='" + agent.getAgentDynamicState() + "', office="
				+ office + " WHERE extension=" + agentExtension;
		// System.out.println(statement);
